local colors_util = require("afcolors.utils.colors")
local color_palette = require("afcolors.core.color_palette")
...
local function get_base()
	...
	return {
		...
		StatusLine = { fg = cp.white, bg = cp.black2 }, -- status line of current window
		StatusLineNC = { fg = cp.black4, bg = cp.black2 }, -- status lines of not-current windows Note: if this is equal to "StatusLine" Vim will use "^^^" in the status line of the current window.
...
local function get_integrations()
	...
	for integration in pairs(integrations) do
		...
		if cot then
			final_integrations = vim.tbl_deep_extend(
				"force",
				final_integrations,
				require("afcolors.core.integrations." .. integration).get(color_palette)
			)
		end
	end
	...
	final_integrations = vim.tbl_deep_extend(
		"force",
		final_integrations,
		require("afcolors.core.remaps").get_hig_remaps() or {}
	)
...
function M.apply()
	_G.cnf = require("afcolors.config").options
